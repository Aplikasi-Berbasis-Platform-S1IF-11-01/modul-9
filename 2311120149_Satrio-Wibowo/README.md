...
1. **PHP (Hypertext Preprocessor):** Bahasa pemrograman *server-side* yang digunakan untuk menciptakan halaman web dinamis. PHP memproses logika bisnis di server sebelum mengirimkan hasilnya dalam bentuk HTML ke browser pengguna.
2. **Array Asosiatif:** Struktur data yang memetakan nilai ke kunci (key) bertipe string. Dalam proyek ini, array asosiatif digunakan untuk menyimpan entitas mahasiswa yang memiliki atribut beragam seperti `nama`, `nim`, dan komponen nilai.
3. **Fungsi (Function):** Blok kode modular yang dapat dipanggil berulang kali. Fungsi seperti `hitungNilaiAkhir()` digunakan untuk memisahkan logika kalkulasi dari kode tampilan (HTML).*.
4. **Struktur Kontrol (Kondisional & Perulangan):
- Kondisional (`if/elseif/else`): Digunakan untuk menentukan klasifikasi huruf (Grade) dan ambang batas kelulusan.
- Perulangan (`foreach`): Digunakan untuk mengiterasi data dalam array mahasiswa secara otomatis untuk merender baris tabel HTML.

---

## 1. Implementasi Persyaratan Tugas (Kebutuhan Sistem)

Program ini mengintegrasikan seluruh elemen PHP yang diminta dalam satu file fungsional yang responsif dan estetis.

---

### 1.1 Array Asosiatif untuk Menyimpan Data Mahasiswa (4 Data)

Data disimpan dalam array multidimensi. Setiap elemen adalah array asosiatif yang menampung minimal 3 data mahasiswa (dalam contoh ini menggunakan 4 data).
...
Fungsi ini menghitung nilai akhir dengan bobot: Tugas 30%, UTS 30%, dan UAS 40%.
```php
<?php
...
Menggunakan operator perbandingan untuk menentukan kelulusan (batas $\geq 65$).
